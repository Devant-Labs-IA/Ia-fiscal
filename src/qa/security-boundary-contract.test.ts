...
describe("contrato estático das fronteiras fiscais", () => {
  ...
  it("não converte administração técnica em vínculo fiscal municipal", () => {
    ...
    expect(authContext).toContain('.from("municipality_memberships")');
    expect(authContext).toContain("role: selected.role");
    expect(authContext).toContain("platformAdmin: true");
    expect(authContext).toContain(
      "municipalityContexts.find((item) => item.id === municipalityId)",
    );
    expect(authContext).toContain("await queryClient.cancelQueries()");
