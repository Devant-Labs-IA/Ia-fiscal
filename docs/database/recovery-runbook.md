...
2. Validar `supabase/baseline/remote-manifest.json` contra as 36 migrações aplicadas.
...
1. Confirmar que existem exatamente 36 arquivos `*.sql` em `supabase/migrations/`, todos aplicados.
2. Recalcular tamanho e SHA-256 e confrontar com `local_bytes` e `local_sha256` do manifesto.
...
5. Ao final, confirmar que o histórico do ambiente descartável contém as 36 versões e nomes.
...
1. os 36 checksums do baseline aplicado passam;
2. o replay em banco descartável termina sem intervenção artificial;
3. as 36 versões aparecem no histórico reconstruído;
