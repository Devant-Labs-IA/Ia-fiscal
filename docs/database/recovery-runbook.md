...
2. Validar `supabase/baseline/remote-manifest.json` contra os 33 arquivos locais.
...
1. Confirmar que existem exatamente 33 arquivos `*.sql` em `supabase/migrations/`.
2. Recalcular tamanho e SHA-256 de cada arquivo e confrontar com `local_bytes` e `local_sha256` do manifesto.
...
5. Ao final, confirmar que o histórico do ambiente descartável contém as mesmas 33 versões e nomes.
...
1. os 33 checksums locais passam;
2. o replay em banco descartável termina sem intervenção artificial;
3. as 33 versões aparecem no histórico reconstruído;
