...
const ACTIONS = new Set<SigisAction>([
  "overview",
  "taxpayer",
  "regime",
  "debts",
...
async function sigisHeaders(): Promise<Record<string, string>> {
  const clientId = Deno.env.get("SIGIS_CLIENT_ID")?.trim();
  const clientSecret = Deno.env.get("SIGIS_CLIENT_SECRET")?.trim();
  const tokenUrl = Deno.env.get("SIGIS_TOKEN_URL")?.trim();
  ...
  if (clientId && clientSecret && tokenUrl) {
    ...
    const scope = Deno.env.get("SIGIS_SCOPE")?.trim();
    ...
    if (!response.ok) throw new Error("sigis_oauth_failed");
    const payload = asObject(await response.json());
    const token = stringValue(payload.access_token);
    if (!token) throw new Error("sigis_oauth_token_missing");
  ...
  const apiKey = Deno.env.get("SIGIS_API_KEY")?.trim();
  if (apiKey) {
    const headerName = (Deno.env.get("SIGIS_API_KEY_HEADER")?.trim() || "x-api-key").toLowerCase();
    if (!/^[a-z0-9-]{2,80}$/.test(headerName)) throw new Error("invalid_sigis_api_key_header");
    return { [headerName]: apiKey };
  }

  throw new Error("sigis_credentials_missing");
}

function buildPath(
  prefix: string,
  action: Exclude<SigisAction, "overview">,
  taxId: string,
  ...
  const pathByAction: Record<Exclude<SigisAction, "overview">, string> = {
    taxpayer: base,
    regime: `${base}/regime`,
    debts: `${base}/debitos`,
...
async function fetchSigis(
  ...
  const response = await fetch(new URL(path, `${baseUrl.replace(/\/$/, "")}/`), {
    headers: {
      ...
      "user-agent": "ia-fiscal-sigis-gateway/1.0",
    },
  ...
  if (Number.isFinite(contentLength) && contentLength > 1_500_000) {
    return { ok: false, status: 502, data: { error: "sigis_response_too_large" } };
  }
...
Deno.serve(async (request: Request) => {
  ...
  const action = stringValue(body.action).trim() as SigisAction;
  ...
  const { data: accessData, error: accessError } = await userClient.rpc(
    "ia_sigis_resolve_access" as never,
    {
  ...
  if (accessError) {
    ...
    return json(request, denied ? 403 : 404, {
      error: denied ? "sigis_access_denied" : "sigis_taxpayer_not_found",
  ...
  const baseUrl = Deno.env.get("SIGIS_BASE_URL")?.trim();
  if (!baseUrl) {
    return json(request, 503, {
      error: "sigis_not_configured",
      configured: false,
      correlation_id: correlationId,
    });
  }
  ...
  try {
    authHeaders = await sigisHeaders();
  } catch (error) {
    return json(request, 503, {
      error: error instanceof Error ? error.message : "sigis_credentials_missing",
      configured: false,
      correlation_id: correlationId,
    });
  }

  const prefix = (Deno.env.get("SIGIS_API_PREFIX")?.trim() || "/api/v1").replace(/\/$/, "");
  const execute = (requestedAction: Exclude<SigisAction, "overview">) =>
    fetchSigis(
      baseUrl,
  ...
  if (action === "overview") {
    ...
    if (!upstream.ok) {
      return json(request, upstream.status === 404 ? 404 : 502, {
        error: upstream.status === 404 ? "sigis_record_not_found" : "sigis_upstream_error",
