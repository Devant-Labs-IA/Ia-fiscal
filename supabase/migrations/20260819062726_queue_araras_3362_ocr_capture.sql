-- Controlled one-shot capture of the official scanned PDF for Araras Law
-- 3.362/2001.  The ingest contract preserves the raw artifact as WORM and
-- queues governed OCR; it never approves or publishes legal content.
...
begin
  ...
  -- A previously captured official artifact makes this release step a no-op.
  if exists (
    select 1
  ...
  insert into private.knowledge_scheduler_dispatch_events (
    ...
    jsonb_build_object(
      'lease_seconds', 120,
      'attempt', 1,
      'trigger', 'phase2_araras_3362_controlled_capture',
